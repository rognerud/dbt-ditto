...
## Installing it as a dbt package

〃 is the ditto mark: *same as above*. That is the whole idea — a description is
written once and every column downstream that means the same thing says ditto.
...
Be clear about what a dbt package can be here: **dbt's Jinja cannot open a file
or run a program**, so no dbt package writes YAML. The ones that appear to are
shipping a Python script beside the macros and asking you to run it, which is
what `packaging/dbt-ditto/run.py` does — it finds `dbt-ditto` on `PATH`, falls
back to `uvx dbt-ditto`, and otherwise says how to install it.

What the macros *can* do without any binary at all is read the same manifest and
print the answer, directives and ambiguity warnings included:
  ...
  - name: dim_customers
    columns:
      - name: first_name
        ...
        meta:
          osmosis_progenitor: seed.platform.raw_customers
# warning: dim_customers.broken_pointer: directive 'raw_customers.no_such_column'
#          has no column 'no_such_column'
```

The binary knows strictly more: it also reads `catalog.json`, so it adds columns
the YAML never listed, removes ones the warehouse no longer has, writes
`data_type`, and inherits across separately parsed projects — none of which is
visible from the manifest alone.

See [`packaging/dbt-ditto/README.md`](../packaging/dbt-ditto/README.md), which
also covers publishing to the dbt package hub.
