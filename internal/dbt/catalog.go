...
// Lookup returns the catalog entry describing a node. The unique_id is tried
// first; failing that the relation is matched on schema and name, which is how
// dbt-osmosis finds an entry and is what makes a catalog generated by a
// different project still usable.
...
// Invalidate drops the relation index, so an entry added after the first
// Lookup is still found by schema and name. Entries are only ever added before
// resolution starts — by a source provider, which writes what a catalog would
// have said about a relation `dbt docs generate` never reached.
