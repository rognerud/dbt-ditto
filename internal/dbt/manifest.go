...
// ColumnConfig is the `config:` block dbt >= 1.9.6 allows on a column.
...
type Column struct {
	...
	// ExtraColumnKeys names them. `policy_tags` is the motivating case.
...
// top-level first, matching the order dbt itself reports.
...
type Node struct {
	...
	// Order is the node's position in the manifest, which is the order dbt writes
	// YAML entries in, so a schema file can be laid out the same way.
	Order int `json:"-"`

	// colIndex maps a lower-cased column name to its entry: a linear scan here
	// turns resolve quadratic on wide models.
...
func (n *Node) Column(name string, fold bool) *Column {
	...
	n.colIndexOnce.Do(func() {
		...
		for k, c := range n.Columns {
			// An exact key wins over a case variant.
...
// it carries no knowledge, but an ancestor that has the column still claims it
// for its generation.
var emptyColumn = &Column{}

// EffectiveColumn is Column, widened to the columns the warehouse reports even when
...
// the first lookup is still found. Only ever added before resolution starts.
...
// SchemaFile returns the repo-relative path of the YAML file documenting the node, and
// whether one exists.
...
// dbt Cloud serves artifacts gzipped. The close function releases both.
...
// `config:` blocks, where dbt-osmosis switches its output shape.
var configBlockMinVersion = [3]int{1, 9, 6}

// WantsConfigBlock reports whether column meta and tags nest under `config:`, inferred
